		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("readarr_indexer_torznab.test", "minimum_seeders", "1"),
					resource.TestCheckResourceAttrSet("readarr_indexer_torznab.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("readarr_indexer_torznab.test", "minimum_seeders", "2"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "readarr_indexer_torznab.test",
				ImportState:       true,
...
func testAccIndexerTorznabResourceConfig(name string, seeders int) string {
	return fmt.Sprintf(`
	resource "readarr_indexer_torznab" "test" {
