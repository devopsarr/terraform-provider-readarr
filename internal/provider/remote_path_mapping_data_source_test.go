...
func TestAccRemotePathMappingDataSource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Read testing
			{
				Config: testAccRemotePathMappingResourceConfig("dataTest", "/test2/") + testAccRemotePathMappingDataSourceConfig("readarr_remote_path_mapping.test.id"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.readarr_remote_path_mapping.test", "id"),
					resource.TestCheckResourceAttr("data.readarr_remote_path_mapping.test", "host", "dataTest")),
			},
		},
	})
}
