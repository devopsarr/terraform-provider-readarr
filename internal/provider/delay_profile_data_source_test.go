		...
		Steps: []resource.TestStep{
			...
			// Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.readarr_delay_profile.test", "id"),
					resource.TestCheckResourceAttr("data.readarr_delay_profile.test", "enable_usenet", "true")),
			},
		},
	})
}
...
func testAccDelayProfileDataSourceConfig(id int) string {
	return fmt.Sprintf(`
	data "readarr_delay_profile" "test" {
		id = %d
	}
	`, id)
}
