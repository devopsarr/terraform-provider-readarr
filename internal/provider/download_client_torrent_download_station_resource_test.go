		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("readarr_download_client_torrent_download_station.test", "use_ssl", "false"),
					resource.TestCheckResourceAttrSet("readarr_download_client_torrent_download_station.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("readarr_download_client_torrent_download_station.test", "use_ssl", "true"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "readarr_download_client_torrent_download_station.test",
				ImportState:       true,
...
func testAccDownloadClientTorrentDownloadStationResourceConfig(name, ssl string) string {
	return fmt.Sprintf(`
	resource "readarr_download_client_torrent_download_station" "test" {
		enable = false
		use_ssl = %s
		priority = 1
		name = "%s"
