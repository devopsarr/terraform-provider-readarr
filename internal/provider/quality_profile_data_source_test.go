...
func TestAccQualityProfileDataSource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Read testing
			{
				Config: testAccQualityProfileDataSourceConfig("eBook"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.readarr_quality_profile.test", "id"),
					resource.TestCheckResourceAttr("data.readarr_quality_profile.test", "cutoff", "2")),
			},
		},
	})
}
...
func testAccQualityProfileDataSourceConfig(name string) string {
	return fmt.Sprintf(`
	data "readarr_quality_profile" "test" {
		name = "%s"
	}
	`, name)
}
